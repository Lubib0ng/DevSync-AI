...
1. [vercel.com](https://vercel.com) 가입 후 GitHub 저장소 연결
2. **Settings → Tokens** 에서 토큰 발급
3. GitHub 저장소 **Settings → Secrets → Actions** 에 추가:

| Secret 이름 | 값 |
|------------|-----|
| `VERCEL_TOKEN` | Vercel 토큰 |
| `VERCEL_ORG_ID` | Vercel 팀/개인 ID (`vercel whoami` 또는 대시보드) |
| `VERCEL_PROJECT_ID` | Vercel 프로젝트 ID (프로젝트 Settings에서 확인) |

### 2단계 — Render 설정 (백엔드)

1. [render.com](https://render.com) 가입 후 **New Web Service** 생성
2. GitHub 저장소 연결, 아래 설정 입력:
   - **Build Command**: `pip install -r backend/requirements.txt`
   - **Start Command**: `uvicorn backend.main:app --host 0.0.0.0 --port $PORT`
3. **Environment Variables** 에 API 키 입력:
   - `GROQ_API_KEY`
   - `GEMINI_API_KEY` (선택)
   - `ANTHROPIC_API_KEY` (선택)
4. **Settings → Deploy Hook URL** 복사
